...
- **`mcp__coach__checkin`** then returns a **reconstructed readiness** when the native score is absent: it weights the same factors Garmin documents — **sleep score + recovery/load (ACWR)** as primary drivers, then **HRV, sleep history, sleep regularity, stress** as secondary — onto the same 0–100 / Poor→Prime scale, and reports the factor breakdown. The **HRV factor**, when a personal history exists, uses the sports-science standard: **ln(RMSSD) of last night vs a rolling baseline with a ±0.5·SD smallest-worthwhile-change band** (Plews/Buchheit), falling back to Garmin's 7-day-avg-vs-balanced-band, then the status label. **Sleep regularity** (SD of recent midsleep times) is included because erratic sleep timing blunts recovery independently of duration. Both come from the stored `garmin_raw` history — no extra fetch. It deliberately does **not** fold in Body Battery or Training Status (they're composites of these same signals — that would double-count); those stay as context. See `adaptive.md` → "Native vs. reconstructed readiness" for how to read the factors.
